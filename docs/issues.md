...
### 29. ~~Если отправлять сообщения очень быстро, некоторые из них могут не отобразиться у собеседника, но отображаются, если обновить страницу~~ ✅ ИСПРАВЛЕНО (2026-03-09)

- **Файл:** `messenger/src/ws/manager.py`, `web/index.html`
- **Категория:** Bug (Race Condition)

При быстрой отправке сообщений параллельные HTTP-запросы вызывали `broadcast_to_users` конкурентно, что приводило к одновременной записи в один и тот же WebSocket из разных корутин. Без синхронизации WebSocket-фреймы перемешивались и сообщения терялись. На фронтенде `sendMessage()` вызывалась повторно до завершения предыдущего вызова, создавая гонку между POST-ответом и WebSocket-событием.

**Исправление:**
- **Бэкенд (`messenger/src/ws/manager.py`):** Добавлен `asyncio.Lock` на каждое WebSocket-соединение. Метод `_send_to_user` захватывает lock перед `send_json()`, исключая конкурентную запись в один сокет. Метод `broadcast_to_users` теперь использует `asyncio.gather()` для параллельной отправки разным пользователям, сохраняя при этом сериализованную запись в каждый отдельный сокет.
- **Фронтенд (`web/index.html`):** Функция `sendMessage()` переписана с использованием цепочки промисов (`sendQueue`). Каждый вызов ставится в очередь и выполняется только после завершения предыдущего, что исключает конкурентные POST-запросы и гонку с WebSocket-событиями.

### 30. ~~Не работает скролл. Если написать больше 10 сообщений (для моего экрана), становится нужен скролл, но его нет~~ ✅ ИСПРАВЛЕНО (2026-03-09)

- **Файл:** `web/index.html`
- **Категория:** Bug (UI)

Контейнер сообщений `.messages` имел `overflow-y: auto`, но flex-родители не имели `min-height: 0`, из-за чего flex-элементы не могли сжиматься меньше размера своего содержимого. Контейнер сообщений рос вместе с содержимым вместо отображения скроллбара.

**Исправление:** Добавлен `min-height: 0` на ключевые flex-контейнеры в цепочке от `.main` до `.messages`:
- `.main` — добавлен `min-height:0` (flex-child `#app-screen`)
- `.chat` — добавлен `min-height:0` (flex-child `.main`)
- `#chat-active` — добавлен `min-height:0` (flex-child `.chat`)
- `.messages` — добавлен `min-height:0` (flex-child `#chat-active`)

Это позволяет flex-контейнерам корректно сжиматься, и `overflow-y: auto` на `.messages` теперь работает как ожидается. Автоскролл вниз при получении нового сообщения (`scrollTop = scrollHeight`) уже был реализован в функциях `appendMessage()` и `loadMessages()`.

### 31. ~~При логине в новый аккаунт появляется чат из старого аккаунта, но после обновления страницы пропадает~~ ✅ ИСПРАВЛЕНО (2026-03-09)

- **Файл:** `web/index.html`
- **Категория:** Bug (UI State Management)

При логауте/логине не очищалось состояние UI: список диалогов, сообщения, активный чат оставались в DOM от предыдущего аккаунта. WebSocket-соединение от старого аккаунта не закрывалось корректно (таймер реконнекта мог переподключить старый сокет). Очередь отправки сообщений (`sendQueue`) не сбрасывалась.

**Исправление:** Добавлена функция `resetAppState()`, которая выполняет полную очистку состояния приложения:
- Закрывает текущий WebSocket и отменяет таймер автоматического переподключения (`wsReconnectTimer`)
- Сбрасывает все переменные состояния (`currentUserId`, `currentUserLogin`, `currentDialogId`, `dialogs`, `sendQueue`)
- Очищает DOM: список диалогов, область сообщений, заголовок чата, имя пользователя, поля ввода
- Возвращает UI в исходное состояние: скрывает активный чат, показывает плейсхолдер, убирает mobile-скрытие сайдбара

Функция вызывается из `doLogout()` (перед показом экрана авторизации) и из `enterApp()` (перед загрузкой данных нового пользователя), что гарантирует чистое состояние при смене аккаунта.

### 32 С мобильного устройства нельзя сделать абзац, сообщения отправляются на энтер ✅ ИСПРАВЛЕНО (2026-03-09)

### 33 С ПК нельзя сделать абзац, нужно добавить функционал абзаца на шифт+ентер ✅ ИСПРАВЛЕНО (2026-03-09)

### 34 ~~Чтобы увидеть новые сообщения в чате часто нужно обновить страницу. Нужно сделать, чтобы пользователь видел новые сообщения и чаты без обновления страницы~~ ✅ ИСПРАВЛЕНО (2026-03-10)

- **Файлы:** `web/index.html`, `messenger/src/schemas/v1/dialogs.py`, `messenger/src/repositories/dialogs.py`, `messenger/src/api/v1/dialogs.py`, `messenger/src/ws/manager.py`, `messenger/src/ws/pubsub.py`, `messenger/src/main.py`, `messenger/src/core/config.py`
- **Категория:** Bug (Real-time Updates)

**Проблема (часть 1 -- фронтенд):** При отключении и переподключении WebSocket данные не синхронизировались — сообщения, полученные за время разрыва, не отображались. Ошибки в обработчике `ws.onmessage` молча проглатывались (`catch (e) {}`), скрывая проблемы. Список диалогов не показывал превью последнего сообщения.

**Проблема (часть 2 -- межворкерная доставка):** Gunicorn запускает несколько воркеров (`workers = cpu_count() * 2 + 1`). Каждый воркер имеет изолированный in-memory `ConnectionManager`. Когда POST-запрос на отправку сообщения попадает в один воркер, а WebSocket-соединение получателя зарегистрировано в другом воркере, `broadcast_to_users` не находит соединений и сообщение не доставляется в реальном времени.

**Исправление (часть 1 -- фронтенд):**
- **Синхронизация при переподключении WebSocket (`web/index.html`):** Добавлен обработчик `ws.onopen`, который при каждом подключении/переподключении перезагружает список диалогов (`loadDialogs()`) и сообщения текущего чата (`loadMessages()`). Это гарантирует, что пропущенные во время разрыва сообщения будут загружены.
- **Логирование ошибок WebSocket (`web/index.html`):** Пустой `catch (e) {}` заменён на `catch (e) { console.error('WS message error:', e); }` — ошибки парсинга или рендеринга больше не проглатываются.
- **Превью последнего сообщения в списке диалогов (backend + frontend):** Добавлены поля `last_message_text` и `last_message_at` в `DialogResponse`. В репозитории добавлен метод `get_last_messages()` с SQL-подзапросом для получения последнего сообщения каждого диалога. Фронтенд обновлён для отображения текста и времени последнего сообщения в списке диалогов. При получении нового сообщения через WebSocket превью диалога обновляется в реальном времени (`updateDialogPreview`).

**Исправление (часть 2 -- Redis Pub/Sub для межворкерной рассылки):**
- **`messenger/src/ws/pubsub.py`:** Создан класс `RedisPubSub`, который подписывается на Redis-канал `ws:broadcast` и обеспечивает межворкерную рассылку WebSocket-событий. При вызове `publish(user_ids, payload)` сообщение сериализуется через `orjson` и публикуется в Redis. Все воркеры-подписчики получают сообщение и рассылают его своим локальным WebSocket-соединениям через `local_broadcast_to_users`. При ошибке Redis происходит fallback на локальную отправку.
- **`messenger/src/ws/manager.py`:** Метод `broadcast_to_users` теперь публикует через Redis Pub/Sub вместо прямой отправки. Добавлен метод `local_broadcast_to_users` для рассылки только локальным соединениям текущего воркера (вызывается из Pub/Sub subscriber). Метод `get_connected_user_ids` работает без изменений — возвращает только локальные соединения.
- **`messenger/src/main.py`:** В lifespan добавлен запуск и остановка `RedisPubSub`. При старте каждого воркера создаётся подписка на Redis-канал.
- **`messenger/src/core/config.py`:** Добавлены настройки `redis_host`, `redis_port`, `redis_password` и property `redis_url`.
- **`messenger/docker-compose.yml`:** Добавлен сервис `redis_messenger` (Redis 7.4.1) с `protected-mode yes` и паролем через переменную окружения.
- **`messenger/requirements.txt`:** Добавлена зависимость `redis[hiredis]>=5.0.0`.

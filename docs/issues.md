...
- **Файлы:** `web/index.html`, `messenger/src/schemas/v1/dialogs.py`, `messenger/src/repositories/dialogs.py`, `messenger/src/api/v1/dialogs.py`
- **Категория:** Bug (Real-time Updates)

При отключении и переподключении WebSocket данные не синхронизировались — сообщения, полученные за время разрыва, не отображались. Ошибки в обработчике `ws.onmessage` молча проглатывались (`catch (e) {}`), скрывая проблемы. Список диалогов не показывал превью последнего сообщения.

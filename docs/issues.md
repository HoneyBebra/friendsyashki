...
### 29. Если отправлять сообщения очень быстро, некоторые из них могут не отобразиться у собеседника, но отображаются, если обновить страницу

### 30. Не работает скролл. Если написать больше 10 сообщений (для моего экрана), становится нужен скролл, но его нет

### 31. При логине в новый аккаунт появляется чат из старого аккаунта, но после обновления страницы пропадает. Баг наверное только фронтенда, но я не уверен. Для этой проблемы есть файл-скриншот @issue-31.png
